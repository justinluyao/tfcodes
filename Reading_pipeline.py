...
#multi_threads raeading
    data_list = [read_and_decode_cpm(tfr_queue, img_size, num_joints, center_radius) for _ in range(2 * len(tfr_path))]
...
def read_and_decode_cpm(tfr_queue, img_size, num_joints, center_radius):
    ...
    heatmap = tf.reshape(features['heatmaps'], [img_size, img_size, (num_of_classes)])

    queue_images, queue_labels, queue_orig_images = preprocessing()
